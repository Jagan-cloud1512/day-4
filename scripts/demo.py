"""The whole project in one command, for the classroom.
    ...
    python -m scripts.demo --real     # the same two questions on Groq (about 12 model calls)
    python -m scripts.demo --crash    # kill the run right after the booking, resume, count
...
QUESTIONS = [
    ("22CS045", "Is an oscilloscope available? If it is, book it for me and send me a text."),
    ("22IT017", "Can I book an Arduino Mega Kit?"),
]
...
def main() -> None:
    ...
    if a.crash:
        ...
        print(f"{GREEN}PASS: one new booking, one text{RESET}" if ok else f"{RED}FAIL: duplicates{RESET}")
